...
**Hey you, do you read this part "Just to learn (always in development mode)" in project's title?** 
...
```bash
sam local invoke SubscribersFunction --event event.json
```
...
aws cloudformation describe-stacks \
    --stack-name subscribers \
    --query 'Stacks[].Outputs[?OutputKey==`SubscribersApi`]' \
    --output table
``` 
...
```bash
sam logs -n SubscribersFunction --stack-name subscribers --tail
```
...
```bash
aws cloudformation delete-stack --stack-name subscribers
```
...
* Invoke with ``sam local invoke SubscribersFunction --event event.json``
* Update tests

...
```bash
aws cloudformation create-stack --stack-name subscribers-pipeline --template-body file://pipeline.yaml --capabilities CAPABILITY_NAMED_IAM
```
...
```bash
aws cloudformation describe-stacks --stack-name subscribers-pipeline --query 'Stacks[].Outputs'
```
